...
function App() {
  ...
  const handleHandshake = (msg: HandshakeMsg) => {
    ...
    if (!peerPubKey.current || peerPubKey.current !== msg.pubKey) {
      ...
      sharedSecret.current = deriveSharedSecret(
        keyPair.current.priv,
        hexToBytes(msg.pubKey)
      );
  ...
  const handleChat = (msg: ChatMsg) => {
    ...
    try {
      ...
      setMessages((msgs) => [
        ...msgs,
        { from: 'system', text: 'Failed to decrypt message' },
      ]);
    }
  };
  ...
  const sendMessage = () => {
    ...
    const { salt, nonce, ciphertext } = encryptMessage(
      sharedSecret.current,
      input
    );
    socket.emit('chat', {
      ...
      <div style={{ width: '70vw', maxWidth: 900 }}>
        <div className='card shadow-sm mx-auto'>
          <div className='card-header bg-primary text-white d-flex align-items-center justify-content-between'>
            <span>
              Secure E2EE Chat{' '}
              <small className='ms-2'>(ECDH + HKDF + ChaCha20)</small>
            </span>
            <span
              className={
                handshakeDone
          ...
          <div className='card-body' style={{ minHeight: 400 }}>
            <div className='mb-3'>
              <div
                className={`alert ${
                  handshakeDone ? 'alert-success' : 'alert-warning'
                } py-2 mb-2`}
                role='alert'
              >
                {status}
              ...
              {!joined ? (
                ...
                <>
                  <div className='d-flex align-items-center gap-2 mb-2'>
                    ...
                    >
                      <i className='bi bi-arrow-repeat me-1'></i>Change Shared
                      Secret
                    </button>
                  </div>
                  <div
